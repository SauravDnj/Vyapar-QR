...
const ACCENT = 'var(--theme-accent,#0e7c66)';
...
export function MinimalTheme({
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-14 px-6 py-16 text-gray-900" style={accentColorStyle(accentColor)}>
      <Reveal>
        <header
          className={`flex flex-col items-center gap-3 text-center ${hero.backgroundImageUrl ? 'relative overflow-hidden rounded-2xl px-6 py-14 text-white' : ''}`}
          style={
        ...
        >
          ...
          <div className={hero.backgroundImageUrl ? 'relative z-10 flex flex-col items-center gap-3' : 'contents'}>
            {hero.logoUrl ? (
              <img src={hero.logoUrl} alt={businessName} className="h-16 w-16 rounded-full object-cover" />
            ) : null}
            <h1 className="text-2xl font-semibold tracking-tight">{orDefault(hero.headline, businessName)}</h1>
            {hero.tagline ? <p className={hero.backgroundImageUrl ? 'text-white/80' : 'text-gray-500'}>{hero.tagline}</p> : null}
          </div>
        </header>
      </Reveal>
      ...
      {about.description || about.address || about.hours || about.phone ? (
        <Reveal>
          <section className="flex flex-col gap-2 text-center text-sm text-gray-600">
      ...
      {galleryImages.length > 0 ? (
        <Reveal>
          <section className="flex flex-col gap-4">
            <h2 className="text-center text-sm font-medium uppercase tracking-wide text-gray-400">Gallery</h2>
      ...
      {reviewConfig?.reviewLink || slug ? (
        <Reveal>
          <section className="flex flex-col items-center gap-4 text-center">
            <h2 className="text-sm font-medium uppercase tracking-wide text-gray-400">{orDefault(reviewsCopy.heading, 'Rate us')}</h2>
      ...
      {contact.heading || about.phone || slug ? (
        <Reveal>
          <section className="flex flex-col items-center gap-4 text-center text-sm text-gray-600">
            <h2 className="text-sm font-medium uppercase tracking-wide text-gray-400">{orDefault(contact.heading, 'Get in touch')}</h2>
            {about.phone ? <p>{about.phone}</p> : null}
            {contact.bookingUrl ? (
      ...
      <footer className="flex flex-col items-center gap-2 text-center text-xs text-gray-400">
