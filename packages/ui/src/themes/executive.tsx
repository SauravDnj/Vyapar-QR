...
const ACCENT = 'var(--theme-accent,#c9a24b)';

/** Professional-category theme: dark, understated, gold-accented — for
 * consultants, lawyers, and other high-trust individual practices. */
export function ExecutiveTheme({
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-14 bg-[#111114] px-6 py-20 text-neutral-200" style={accentColorStyle(accentColor)}>
      <Reveal>
        <header
          className={`flex flex-col items-center gap-4 text-center ${hero.backgroundImageUrl ? 'relative overflow-hidden rounded-2xl px-6 py-14' : ''}`}
          style={
        ...
        >
          ...
          <div className={hero.backgroundImageUrl ? 'relative z-10 flex flex-col items-center gap-4' : 'contents'}>
            ...
            <h1 className="text-2xl font-semibold tracking-[0.08em] text-white">{orDefault(hero.headline, businessName)}</h1>
            {hero.tagline ? (
              <p className="text-sm uppercase tracking-[0.15em]" style={{ color: ACCENT }}>
                {hero.tagline}
              </p>
            ) : null}
      ...
      {about.description || about.address || about.hours || about.phone ? (
        <Reveal>
          <section className="flex flex-col gap-2 text-center text-sm leading-relaxed text-neutral-400">
      ...
      {reviewConfig?.reviewLink || slug ? (
        <Reveal>
          <section className="flex flex-col items-center gap-4 border border-[#2a2a30] p-6 text-center">
      ...
      {contact.heading || about.phone || slug ? (
        <Reveal>
          <section className="flex flex-col items-center gap-4 text-center text-sm text-neutral-400">
            <h2 className="text-xs font-medium uppercase tracking-[0.2em]" style={{ color: ACCENT }}>
      ...
      <footer className="flex flex-col items-center gap-2 text-center text-xs text-neutral-600">
