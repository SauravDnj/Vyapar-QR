...
/** Services-category theme: clean corporate two-tone (navy/white), built to
 * read as credible for consultants, agencies, and trade professionals. */
export function TrustlineTheme({
  slug,
  businessName,
  content,
  paymentMethods,
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col text-slate-800" style={accentColorStyle(accentColor)}>
      <Reveal>
        <header
          className={`flex flex-col items-center gap-3 px-6 py-14 text-center text-white ${hero.backgroundImageUrl ? 'relative overflow-hidden' : ''}`}
          style={
        ...
        >
          ...
          <div className={hero.backgroundImageUrl ? 'relative z-10 flex flex-col items-center gap-3' : 'contents'}>
            {hero.logoUrl ? (
              <img src={hero.logoUrl} alt={businessName} className="h-16 w-16 rounded-md object-cover" />
            ) : null}
            <h1 className="text-2xl font-semibold">{orDefault(hero.headline, businessName)}</h1>
            {hero.tagline ? <p className="text-sm text-slate-300">{hero.tagline}</p> : null}
          </div>
        </header>
      </Reveal>
      ...
      <div className="flex flex-col gap-10 px-6 py-12">
        ...
        {menu.fileUrl ? (
          <Reveal>
            <section className="flex flex-col gap-4">
              <h2 className="text-sm font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>{orDefault(menu.heading, 'Menu')}</h2>
        ...
        {galleryImages.length > 0 ? (
          <Reveal>
            <section className="flex flex-col gap-4">
              <h2 className="text-sm font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>Gallery</h2>
        ...
        {reviewConfig?.reviewLink || slug ? (
          <Reveal>
            <section className="flex flex-col items-center gap-4 rounded border border-slate-200 p-6 text-center">
              <h2 className="text-sm font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>
                {orDefault(reviewsCopy.heading, 'Rate us')}
              </h2>
              {reviewConfig?.avgRatingCached ? <p className="text-xl font-semibold">★ {reviewConfig.avgRatingCached}</p> : null}
        ...
        {contact.heading || about.phone || slug ? (
          <Reveal>
            <section className="flex flex-col items-center gap-4 text-center text-sm text-slate-600">
              <h2 className="text-sm font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>
      ...
      <footer className="flex flex-col items-center gap-2 bg-slate-100 py-6 text-center text-xs text-slate-400">
