...
const ACCENT = 'var(--theme-accent,#c0442f)';

/** Restaurant-category theme: warm, appetite-inducing palette; menu-card styling. */
export function SpiceTheme({
  slug,
  businessName,
  content,
  paymentMethods,
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-10 bg-[#fff8f2] px-6 py-14 text-stone-900" style={accentColorStyle(accentColor)}>
      <Reveal>
        <header
          className={`flex flex-col items-center gap-3 rounded-2xl px-6 py-10 text-center text-white ${hero.backgroundImageUrl ? 'relative overflow-hidden' : ''}`}
          style={
        ...
        >
          ...
          <div className={hero.backgroundImageUrl ? 'relative z-10 flex flex-col items-center gap-3' : 'contents'}>
            {hero.logoUrl ? (
              <img src={hero.logoUrl} alt={businessName} className="h-16 w-16 rounded-full border-2 border-white/70 object-cover" />
            ) : null}
            <h1 className="text-2xl font-bold">{orDefault(hero.headline, businessName)}</h1>
            {hero.tagline ? <p className="text-sm text-orange-100">{hero.tagline}</p> : null}
          </div>
        </header>
      </Reveal>
      ...
      {about.description || about.address || about.hours || about.phone ? (
        <Reveal>
          <section className="flex flex-col gap-1.5 rounded-xl border border-[#eddccb] bg-white p-4 text-center text-sm text-stone-600">
      ...
      {menu.fileUrl ? (
        <Reveal>
          <section className="flex flex-col gap-4 rounded-xl border border-[#eddccb] bg-white p-4">
            <h2 className="text-center text-sm font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>
      ...
      {reviewConfig?.reviewLink || slug ? (
        <Reveal>
          <section className="flex flex-col items-center gap-4 rounded-xl bg-white p-6 text-center shadow-sm">
      ...
      {contact.heading || about.phone || slug ? (
        <Reveal>
          <section className="flex flex-col items-center gap-4 rounded-xl border border-[#eddccb] bg-white p-6 text-center text-sm text-stone-600">
            <h2 className="text-sm font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>
